...
export const getVideoUploadUrl = withErrorHandling(async () => {
        ...
        return { videoId: videoResponse.guid, uploadUrl, accessKey: ACCESS_KEYS.streamAccessKey }
})
...
export const getAllVideos = withErrorHandling(async (searchQuery: string = '', sortFilter: string = '', pageNumber: number = 1, pageSize: number = 8) => {
        const session = await auth.api.getSession({ headers: await headers() })
...
export const getAllVideosByUser = withErrorHandling(async (userIdParameter: string, searchQuery: string = '', sortFilter?: string) => {
        ...
        const conditions = [eq(videos.userId, userIdParameter), !isOwner && eq(videos.visibility, 'public'), searchQuery.trim() && ilike(videos.title, `%${searchQuery}%`)].filter(Boolean) as any[]
